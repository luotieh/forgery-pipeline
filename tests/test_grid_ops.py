...
def test_build_grid_row_counts_and_field_conventions(tmp_path):
    ...
    # 裁决3（池分离）回归锚：holdout_generators 缺省 () → hold 池恒空、全部底图落 train
    # 池——每个底图仍产全部 img2img spec 行 + 恰 1 行 outpaint，与池分离前行为一致。
    per_base_i2i = Counter(s.base_id for s in i2i)
    per_base_outp = Counter(s.base_id for s in outp)
    assert len(per_base_i2i) == len(bases) and all(
        c == len(_IMG2IMG) for c in per_base_i2i.values())
    assert len(per_base_outp) == len(bases) and all(c == 1 for c in per_base_outp.values())
