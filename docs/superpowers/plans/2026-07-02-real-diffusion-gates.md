...
- [ ] **Step 1: 写失败测试** `tests/test_backends_real.py`
...
- [ ] **Step 3: 写 `src/forgery_pipeline/backends/real/local_source.py`**
...
- [ ] **Step 4: 改 `registry.py` 的 `get_image_source`**
...
- [ ] **Step 5: 写 `scripts/fetch_real_images.py`**
...
- [ ] **Step 1: 追加失败测试** `tests/test_backends_real.py`
...
- [ ] **Step 3: 整体替换 `src/forgery_pipeline/backends/real/diffusers_gen.py`**
...
- [ ] **Step 4: 改 `registry.py`** —— `get_img2img/get_inpainter/get_segmenter` 加 real 分支
...
- [ ] **Step 1: 更新测试** `tests/test_checking_extractor.py`（`get_extractor("real")` 改为惰性返回实例）
...
- [ ] **Step 3: 替换 `checking/extractor.py` 的 `DiffusersSD2Residual`**（整类替换现有 stub）
...
- [ ] **Step 4: 写 `configs/probe.real.yaml`**
...
- [ ] **Step 5: 改 `pyproject.toml` 的 `[real]` extra**
...
- [ ] **Step 3: 真实 probe 冒烟（n_base=4，触发权重下载）**
...
- [ ] **Step 6: 写 `docs/real_gate_results_2026-07-02.md`**
